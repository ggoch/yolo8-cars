...
def main(input_dir, output_dir, labels, noviz=False):
    ...
    for image_id, filename in enumerate(label_files):
        ...
        label_str_list = []  # 存储 label.txt 的内容
        for shape in label_file.shapes:
            ...
            # 添加 label.txt 字符串
            cls_id = class_name_to_id.get(label, -1)  # 如果类名不存在则返回 -1

            if cls_id != -1:
                points_str = " ".join(map(str, points))
                label_str_list.append(f"{cls_id-1} {points_str}")

        segmentations = dict(segmentations)

        label_txt_path = osp.join(output_dir, "Labels", base + ".txt")
